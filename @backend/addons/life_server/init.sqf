...
PHX_radio_havToNHS = radioChannelCreate [[1, 1, 0, 0.8], "CSAT to NHS [Text]", "%UNIT_NAME", []];
...
// Only allows super caps to be active after 8pm
PHX_superCapsActive = false;
if (_hour >= 20) then {
    PHX_superCapsActive = true;
};
...
} forEach [fed_bank, evidence_locker, casino_box];
