/*
    ...
    Patch: Injects HUD for players in zone via PHX_fnc_SC_ui and unloads on loss
*/
    ...
    private _thread = _x spawn {
        ...
        for "_i" from 0 to 1 step 0 do {
            ...
            private _dominatingGroup = (
                [_capZone, independent, false, "!isNil {(group _x) getVariable 'gang_name'} || {(side _x) in [west, east]}"]
                call PHX(unitsInZone)
            ) apply { group _x } param [0, grpNull];

            if (!isNull _dominatingGroup && {
                count ([_capZone, _dominatingGroup, false, "(!isNil {(group _x) getVariable 'gang_name'} || (side _x) in [west, east]) && {!(side _x isEqualTo independent)}"]
                ...
                switch true do {
                    case ((!_captured || {_isZoneOwner}) && {_zoneProgress >= 1}): {
                        if !(_isZoneOwner) then {
                            ...
                            _obj setVariable ["zOwnerName", switch (side _dominatingGroup) do {
                                case west: { "The APC" };
                                case east: { "CSAT" };
                                default { _dominatingGroup getVariable ["gang_name", "Unknown Group"] };
                            }, true];

                            [3, format [
                                "<t color='#ff0000' size='1.4px'>%1</t><br/><t color='#ffffff' size='1px'>%2 have taken control over the %1!</t>",
                                _displayName,
                                switch (side _dominatingGroup) do {
                                    case west: { "The APC" };
                                    case east: { "CSAT" };
                                    default { _dominatingGroup getVariable ["gang_name", "Unknown Group"] };
                                }
                            ...
                            [2, format ["Your group has successfully taken over the %1!", _displayName]] remoteExecCall ["life_fnc_broadcast", _captors];
                            ...
                            ["unLoad", _obj] remoteExecCall ["PHX_fnc_SC_ui", _captors];
                        };

                        private _markerFormat = format ["%1 | %2", _displayName,
                            switch (side _dominatingGroup) do {
                                case west: { "APC" };
                                case east: { "CSAT" };
                                default { _dominatingGroup getVariable ["gang_name", "Unknown Group"] };
                    ...
                    default {
                        ...
                        private _clients = [_capZone, _dominatingGroup, true] call PHX(unitsInZone);
                        ["init", _obj] remoteExecCall ["PHX_fnc_SC_ui", _clients];
                ...
                if (isNull _dominatingGroup) then {
                    ...
                    private _zonePlayers = [_capZone, grpNull, true] call PHX(unitsInZone);
                    if (_zonePlayers isNotEqualTo [] && {!isNil "_zonePlayers"}) then {
                        ["unLoad", _obj] remoteExecCall ["PHX_fnc_SC_ui", _zonePlayers];
                    };
                };
            };
        };
