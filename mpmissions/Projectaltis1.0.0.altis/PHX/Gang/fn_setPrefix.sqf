...
if ((_cntTg < 3 || { _cntTg > 6 }) && { !(_newTag isEqualTo "") }) exitWith { ["Your tag must be between 2 - 6 characters long","red"] spawn PHX(notify) };

if (_newTag in ["[APC]", "[NHS]", "[CSAT]", "[SO1]", "[CID]", "[HSS]"]) exitWith { ["Your tag can't include blacklisted words","red"] spawn PHX(notify) };
