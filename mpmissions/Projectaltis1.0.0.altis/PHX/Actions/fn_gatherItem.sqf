...
// Get the zone we're currenly in...
...
private _progress = getArray(_myZone >> "progress");

//Reduce sleep time for 50% faster tick rate
private _sleep = (_progress select 0) * 0.5;
private _increase = _progress select 1;
...
_increase = ([ _increase, (_increase + (_level / 2500)) ] select (_level > 0)); // XP bonus preserved
...
// Gem reward logic
private _gems = getArray(_myZone >> "gems");
if !(_gems isEqualTo []) then {
    private _gem = ((_gems select 0) selectRandomWeighted (_gems select 1));
