...
if ((_object getVariable ["inCapture",false])) exitWith {NOTIFY("Only one person shall capture at once!","red")};
...
_object setVariable ["inCapture",true,true];
...
) exitWith { _object setVariable ["inCapture",false,true]; life_action_inUse = false; };
...
private _gangName = (group player) getVariable ["gang_name", "Unknown"];
private _gangId = (group player) getVariable ["gang_id", -3];
private _playerName = name player;
private _siteType = _type;
private _timeNow = [systemTime, "HH:MM:SS"] call BIS_fnc_timeToString;

[
    "CaptureSite", // Template name in CfgDiscordEmbedBuilder
    [ _playerName, _gangName, _siteType, _timeNow ]
] call DiscordEmbedBuilder_fnc_buildCfg;

/* --- End of Discord Embed Integration --- */

_object setVariable ["inCapture",false,true];
