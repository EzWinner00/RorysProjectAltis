...
if (!(isNil {_house getVariable "inStorage"}) && {!(player getVariable ["isAdmin",false])}) then {
	...
	private _tarGang = _tar getVariable ["gang_uid", -1];
	private _playerGang = player getVariable ["gang_uid", -1];

	// If it's gang-owned and both are in the same gang, allow access
	if (!(_isGang && {_tarGang isEqualTo _playerGang})) then {
		// Wipe variables...
		house = nil;
		house_categories = nil;
		house_current = nil;
