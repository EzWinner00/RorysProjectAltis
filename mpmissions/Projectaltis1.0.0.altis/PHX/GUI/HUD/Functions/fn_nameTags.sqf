	...
	if (life_settings_tagson && { !(isDowned(player)) }) then {
...

		{
			...
			if (!lineIntersects [eyePos player, eyePos _x, player, _x] && {!isNil {_x getVariable "realname"}} && { !(_x getVariable ["hideName", false]) }) then {
				...
				// Masked civilian name override (keep all other tag logic intact)
				if (
					(side _x isEqualTo civilian) &&
					((headgear _x) in _masks || (goggles _x) in _masks || (uniform _x) in _masks)
				) then {
				...
				switch true do {
					...
					case (((side _x) isEqualTo civilian) && { !(isNil {_x getVariable "isPM"}) }): {
						_icon = format["%1data\ui\PM.paa", MISSION_ROOT];
						_subtitle = "Governor";
					};
