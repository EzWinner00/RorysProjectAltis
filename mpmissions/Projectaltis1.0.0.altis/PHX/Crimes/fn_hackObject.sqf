...
private _sleep = 600; //600
if (_crime isEqualTo "Destroyer") then {
    _sleep = 140; //140
} else {
    if (_power) then { _sleep = 800 }; //800
};
...
if (_crime isEqualTo "Casino") exitWith { [] remoteExecCall (["fnc_getCasinoValue", "PHXSRV"] call SOCK_fnc_getRemoteDestination); };
