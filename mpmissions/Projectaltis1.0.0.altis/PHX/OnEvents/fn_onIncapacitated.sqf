...
switch (_type) do {
	case "event": {
		...
		if (!isNull _killer && { isPlayer _killer } && { !(_killer isEqualTo _unit) }) then {
			...
			if ([(position _unit)] call PHX_fnc_isSupercap && { [(position _killer)] call PHX_fnc_isSupercap }) then {
				if !(((group _unit) getVariable ["gang_id", -2]) isEqualTo ((group _killer) getVariable ["gang_id", -3])) then {
					
					private _killmoney = round random [30000,40000,50000];
					[_killmoney, 0, 1, "Supercap Kill"] remoteExecCall ["PHX_fnc_handleMoney", _killer];
					[(format["You earned £%1 for a Supercap kill!",(_killmoney)]),"cyan"] remoteExecCall ["PHX_fnc_notify",_killer];
