...
if (isClass (missionConfigFile >> "CfgProcess" >> _type)) then {
    ...
    _progress = M_CONFIG(getArray,"CfgProcess",_type,"progress");
};
...
private _sleep = _progress select 0; // This value is divided by 100 so 50 = 0.5 / around half a second...
...
_increase = ([
	_increase, 
	(_increase + (_level / 2500))
] select (_level > 0 && { _hasLicense }));

if (_hasLicense) then { _sleep = _sleep * 0.3 }; // If you have the license then the sleep time is 30% of base...
