...
if ((crew _vehicle) isEqualTo [] || { _vehicle isKindOf "B_UAV_01_F" }) then {
    ...
    if (playerSide isEqualTo west || playerSide isEqualTo east || playerSide isEqualTo independent) then {
        ...
        if (_vehicle getVariable ["isAPC",false] && playerSide isEqualTo west ||  {_vehicle getVariable ["isHAV",false] && playerSide isEqualTo east} || {_vehicle getVariable ["isNHS",false] && playerSide isEqualTo independent}) then {
            [(format ["You were paid nothing for impounding a faction %1.", _type]),"red"] call PHX_fnc_notify;
        } else {
