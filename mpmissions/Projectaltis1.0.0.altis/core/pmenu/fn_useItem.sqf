/*
    File: fn_useItem.sqf
    Author: Bryan "Tonic" Boardwine + Patched by Scarso Lewis
...
switch (true) do {
    case (isText(missionConfigFile >> "CfgItems" >> _item >> "Events" >> "onUse")): {
        private _code = getText(missionConfigFile >> "CfgVirtualItems" >> _item >> "Events" >> "onUse");
        private _handled = call compile _code;

        if (_handled isEqualType true && { !_handled }) exitWith {
    ...
    case (_item isEqualTo "siphonpump"): {
        if !(vehicle player isEqualTo player) exitWith {
            ["You can't do this whilst inside a vehicle!", "red"] call PHX_fnc_notify;
        };
    ...
    case (_item isEqualTo "spikeStrip"): {
        if (!isNull life_spikestrip) exitWith {
            ["You already have an active spike strip", "red"] call PHX_fnc_notify;
            closeDialog 0;
        };
    ...
    case (_item isEqualTo "fuelFull"): {
        if !(isNull objectParent player) exitWith {
            ["You can't refuel the vehicle while in it!", "red"] call PHX_fnc_notify;
        };
    ...
    case (_item in ["gpstracker", "gpstracker_remover"]): {
        [cursorObject, (_item isEqualTo "gpstracker")] spawn PHX_fnc_trackerAction;
    };

    case (_item isEqualTo "breathalyser"): {
        if !(isPlayer cursorTarget) exitWith {
            ["You are not looking at anyone!", "red"] call PHX_fnc_notify;
        };
        if !(playerSide in [west, independent]) exitWith {
            ["Only certain people know how to use this!", "red"] call PHX_fnc_notify;
        };
        sleep 2;
