                        ...
                        if (alive cursorObject && { player distance cursorObject < 7 }) then {
                            ...
                            if ([cursorObject, _list] call PHX(isKindOf)) then {
                                ...
                                switch (true) do {
                                    ...
                                    case ((vehicleVarName cursorObject) isEqualTo "casino_box"): {
                                        (casino_box getVariable ["state", [false, false]]) params ["_hacked", "_power"];

                                        if (!_hacked || { isNil { casino_box getVariable "Trunk" } }) then {
                                            ["You can't access the vault until it's security has been hacked or this vault has already been emptied..."] spawn PHX(notify);
                                        } else {
                                            [casino_box] spawn life_fnc_openInventory;
                                        };
                                    };
