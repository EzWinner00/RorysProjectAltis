...
diag_log format ["[CIV EVENT] Client starting event %1 until %2", _event, _endTime];
...
if (!isNull _disp) then {
    ...
    if (!isNull _titleCtrl) then {
        private _instruction = switch (_event) do {
            ...
            case "biggestHit": { " Take the biggest single hit and survive!" };
            default { " Civilian Event In Progress" };
        };
