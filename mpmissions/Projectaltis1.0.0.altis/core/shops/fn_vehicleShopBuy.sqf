...
/*
    ...
    Description:
    Handles buying and renting vehicles with plate validation and spawn logic.
*/
...
if (player getVariable ["isAdmin", false]) exitWith {
    ["You can't buy or rent vehicles while on-duty"] spawn PHX(notify);
};

if ((lbCurSel 2302) isEqualTo -1) exitWith {
    NOTIFY("You did not pick a vehicle!", "red");
    closeDialog 0;
};
...
if (!(_plateManaged)) exitWith {
    ...
    if (playerSide isEqualTo civilian) then {
        ...
        _action = [
            format["Are you sure you wish to %1 this vehicle?", ["rent", "buy"] select _mode],
    ...
    if (!(_action)) exitWith {
        [format["You've chosen to cancel the %1 of this vehicle.", ["rental", "purchase"] select _mode], "red"] call PHX_fnc_notify;
    };
    ...
    if (!_mode) then {
        ...
        // Plate checks for purchased vehicles
        if ((count _plate) > 8 && { !(_plate isEqualTo "") }) exitWith {
            NOTIFY("The number plate must be less than 9 characters", "red")
        };
        if ((count _plate) < 3 && { !(_plate isEqualTo "") }) exitWith {
            NOTIFY("The number plate must be more than 2 characters", "red")
        };
        if ((toLower _plate) in LIFE_SETTINGS(getArray,"blockedWords")) exitWith {
            NOTIFY("You've included a naughty word in your number plate!", "red")
        };
        if (PHX_DonatorLevel < 1 && {!([_plate] call LIFE_fnc_isNumber)} && { !(_plate isEqualTo "") }) exitWith {
            NOTIFY("Only lifetime donators can include letters in their number plates!", "red");
            breakOut "vehicleBuy";
        };
    };
...
if (!_mode) then {
    _vehicle setVariable ["rented", true, true];                                //Rented
    [] call PHX_fnc_addRental;
};
...
if (_mode) then {
    if !(_className in LIFE_SETTINGS(getArray,"vehicleShop_rentalOnly")) then {
        [(getPlayerUID player),playerSide,_vehicle,_colorIndex,1,(HAS_PERK("Scrambler")),_plate,player] remoteExecCall (["fnc_vehicleCreate","TON"] call SOCK_fnc_getRemoteDestination);
    };
